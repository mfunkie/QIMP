...
var universalModule = require('./universal')
...
var GameState = (function(){
	...
	return {player:player.handle,players:self._getPlayers()}
    }
	...
	async.series([self._createQuestionSet,self._initializePlayers],
		     function(err,obj){
			 if(err)
	...
	var generateQuestions = function(index, cb){
	    ...
	    //var correctOutput = vm.runInContext(questions[index].code,context)
	    totalQuestions = self.questions.length
	    body = Object.create(self.QuestionBody)
	    body.input=inputs
	    body.output=correctOutput
	    body.duration=self.settings.timePerQuestion
	    body.textQuestions = []
	    tempArray = []
	    for(i = 0;i<3;++i){
	    ...
	    var gameQuestion = Object.create(self.GameQuestion)
	    gameQuestion.body=body
	    gameQuestion.bodyIndex=correctPosition
	    gameQuestion.questionsIndex=index
	...
	async.parallel([
	    function(callback){schema.loadSettings(function(settingsObj){self.settings = settingsObj
									 callback(null)})},
	    function(callback){
